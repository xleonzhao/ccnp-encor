...
* multicast: 
  ...
  * AllDRouters: **224.0.0.6** or MAC address 01:00:5E:00:00:06. Only designated routers (DRs) uses this address
* 5 packet types
...
* to avoid possible false routes
...
* __Type 5, AS external LSA__: Advertises LSAs for routes that have been redistributed
...
* DR originates this LSA to list all OSPF routers attached to the same LAN segment.
...
* area M type 3 -> area M LSDB, no type 3
  * someone from area 0 generated this type 3, so area 0 already has it
...
* no type 3 learned from area M (M>0) will not be regened
* virtual link is operational complicated
...
* support IPv6
* runs directly on IPv6 w/ link-local addr
...
* multiple instances: OSPFv3 packets carries an Instance ID field
...
* type 1 changed
  * advertising only interface type and cost
  * separation of topology (nodes, links) from prefixes attached to nodes
...
* Step 1.Ensure that the IPv4 interface has an IPv6 address (global or link local) con- figured. 
