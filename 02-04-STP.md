...
* per-VLAN STP (PVST) allows root bridge on different switches or cost port differently peer vlan
  * may cause CPU busy dealing with BPDUs, esp. during convergence

## MST instance

* MST maps new VLAN to existing STP tree (**MST instance**) which was built prior
  * based on where root bridge is
* **MST region**: a group of switches appearing as one (virtual) switch
* **internal spanning tree (IST)**: special MST instance for a MST region, instance 0
...
* pruning VLANs from trunk = allow a set of vlans on this trunk, and other vlans on that trunk -> load balancing
